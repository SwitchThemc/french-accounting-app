...
function run() {
  ...
  addCheck("Contract interview has structured brief UI", app.includes("contractBriefStatus") && app.includes("quick-prompts") && includes("src/styles.css", "contract-brief-panel"), "Contract chat shows extracted facts, missing facts, brief strength, and quick-start prompts.");
  addCheck("Contract AI prompt uses fact ledger", app.includes("buildContractFactLedger") && app.includes("Treat the fact ledger below as source-of-truth") && app.includes("Use Party A's verified company identity exactly"), "AI drafting prompt preserves structured facts and verified company identity.");
  addCheck("Contract register uses clear cards", app.includes("contract-register-grid") && app.includes("contract-card-meta") && includes("src/styles.css", "contract-card-actions"), "Contract register displays readable cards with status, approval, QA, and named actions.");
  addCheck("Contracts can be modified with AI", app.includes("modifyContractWithOllama") && app.includes("applyAiContractModification") && includes("src/styles.css", "contract-ai-modifier"), "Existing contract drafts can be rewritten from a user instruction through the Ollama AI flow.");
  ...
  addCheck("Contract QA blocks approval/finalization", app.includes("Fix contract QA issues before requesting approval") && app.includes("Fix contract QA issues before finalizing") && app.includes("contractReviewStatus"), "Contracts with automatic QA issues cannot proceed to approval or final status.");
  ...
  addCheck("Keyboard focus states are visible", includes("src/styles.css", ":focus-visible") && includes("src/styles.css", ".skip-link"), "Interactive controls expose visible focus rings and a skip link.");
  addCheck("Accounting app icon is used in chrome", app.includes('src="/icon-192.png"') && includes("src/styles.css", ".brand-mark img"), "Auth and sidebar branding use the accounting icon instead of a generic or stale app logo.");
  addCheck("Auth screen explains core value", app.includes("authBenefitOne") && app.includes("authBenefitTwo") && app.includes("authBenefitThree") && includes("src/styles.css", ".auth-benefits"), "The login screen states accounting, PWA scan, and open-source workflow benefits.");
  addCheck("Kbis scan populates company identity", app.includes("parseKbisScan") && app.includes("extractTextFromPdf") && app.includes("registered_address") && fileExists("supabase/migrations/20260604100000_company_kbis_fields.sql"), "Kbis PDF/image uploads can prefill durable company legal identity fields.");
  addCheck("Kbis company identity feeds documents", app.includes("formatCompanyIssuerLines") && app.includes("company.registered_address ||") && app.includes("party_a_address"), "Invoices and contracts use stored Kbis address and identifiers.");
